...
socket.on("file", d => {
  ...
  if (d.type.startsWith("image")) {
    div.innerHTML = `<img src="${d.file}" style="max-width:200px;border-radius:10px;">`;
  } else if (d.type.startsWith("audio")) {
    div.innerHTML = `<audio controls src="${d.file}"></audio>`;
  } else {
    div.innerHTML = `<a href="${d.file}" download>Download</a>`;
  }
